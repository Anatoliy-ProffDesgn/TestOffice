...
ui.custom_model = QtGui.QStandardItemModel()
...
def setData(data_rez):
    ...
    ui.treeView_2.setModel(ui.model_null)
    try:
        ui.custom_model.invisibleRootItem().clearData()
        for item in data_rez:
            root = ui.custom_model.invisibleRootItem()
            root.appendRow([QtGui.QStandardItem(item['Article']),
                            QtGui.QStandardItem(item['Name']),
                            QtGui.QStandardItem(item['Price']),
                            QtGui.QStandardItem(item['Unit']),
                            QtGui.QStandardItem(item['Category'])])
        list_models.append(ui.custom_model)
    except:
        list_models.append(ui.model_null)
...
# ------------------conext menu-------------------------------------------------
# ------------------------------------------------------------------------------
def re_model_treeview(me_model, treeview):
    if len(list_models) == 2:
        list_models.append(treeview.model())
    else:
        if list_models[1] != treeview.model():
            list_models[2] = treeview.model()
    clear_model(treeview.selectionModel(), me_model)
    treeview.setModel(me_model)
...
def showContextMenu(point):
    ...
    action_custom_price.triggered.connect(lambda: re_model_treeview(list_models[1], ui.treeView))
    action_full_price.triggered.connect(lambda: re_model_treeview(list_models[0], ui.treeView))
